...
# Dataset: HyperKvasir segmentation subset
# Runs: 8 hyperparameter variants x 1 seed = 8 runs
# ============================================================
...
RUN_TAG="hyperparameters_hyper_kvasir_seg"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${WORK_ROOT}/bt_unet_outputs/${RUN_TAG}_seed_$(echo "${SEEDS}" | tr ',' '_')}"
RESULT_ARCHIVE="${RESULT_ARCHIVE:-${WORK_ROOT}/${RUN_TAG}_results.zip}"

RUNNER="scripts/run_dapr_hyperparameters.py"
CONFIG_DIR="configs/dapr_hyperparameters"
...
echo "WARNING: 8 runs on HyperKvasir may still exceed 12 hours at 30 epochs."
...
python tools/audit_dapr_hyperparameters.py --config-dir "${CONFIG_DIR}"
...
section "RUN DAPR HYPERPARAMETER SENSITIVITY"
rm -rf "${OUTPUT_ROOT}"
mkdir -p "${OUTPUT_ROOT}"
...
echo "  ${TABLE_DIR}/dapr_hyperparameters.tex"
echo "  ${TABLE_DIR}/dapr_hyperparameter_deltas.csv"
echo "  ${RESULT_ARCHIVE}"
...
echo
cat "${TABLE_DIR}/dapr_hyperparameter_deltas.csv"
echo
cat "${TABLE_DIR}/dapr_hyperparameters.tex"
