...
FAIR_DIR = PROJECT_ROOT / "configs" / "fair"
...
def _fast_test_cfg(model_name: str, cfg: dict):
    ...
    if model_name in {"attention_unet", "dapr_unet"}:
...
def test_fair_model_runs_forward_parse_and_loss(model_name: str):
    torch.manual_seed(123)
    cfg = _fast_test_cfg(model_name, _load_cfg(FAIR_DIR, model_name))
...
def test_fair_model_runs_backward_with_finite_gradients(model_name: str):
    torch.manual_seed(123)
    cfg = _fast_test_cfg(model_name, _load_cfg(FAIR_DIR, model_name))
    model = build_model(model_name, cfg["model"])
    model.train()
...
def test_official_faithful_model_can_run_tiny_trainer_and_evaluator_loop(model_name: str):
    ...
    cfg = _load_cfg(OFFICIAL_FAITHFUL_DIR, model_name)
    model = build_model(model_name, cfg["model"])
