...
RUN_TAG="baseline_kvasir_instrument"
...
MODELS="unet,unetpp,attention_unet,pranet,acsnet,hardnet_mseg,cfanet,polyp_pvt,caranet,hsnet,resunetpp,dapr_unet"
CONFIG_DIR="configs/fair"
...
echo "WARNING: 36 runs on Kvasir-Instrument are very likely to exceed 12 hours at 30 epochs."
...
python tools/audit_baseline_proposal_comparison.py --config-dir "${CONFIG_DIR}"
python tools/audit_baseline_dapr_strict.py --config-dir "${CONFIG_DIR}" --no-forward
python tools/audit_baseline_implementations.py --skip-runtime
...
python scripts/aggregate_training_results.py \
    --output-root "${OUTPUT_ROOT}" \
    --output-path "${TABLE_DIR}/baseline_proposal_training_summary.csv" \
    --models "${MODELS}" \
    --seeds "${SEEDS}" \
    --dataset "${DATASET}" \
...
python scripts/validate_ablation_results.py \
    ...
    --training-summary-path "${TABLE_DIR}/baseline_proposal_training_summary.csv" \
    --output-root "${OUTPUT_ROOT}" \
    --models "${MODELS}" \
    --seeds "${SEEDS}" \
    --dataset "${DATASET}"

python scripts/report_baseline_proposal_comparison.py \
    --summary-path "${TABLE_DIR}/multi_seed_summary.csv" \
    --latex-path "${TABLE_DIR}/baseline_proposal_comparison.tex" \
    --delta-path "${TABLE_DIR}/baseline_proposal_deltas.csv"
